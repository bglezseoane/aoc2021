...
def calculate_final_position(movements: [str], consider_aim: bool = False) -> int:
    ...
    consider_aim : bool
        Consider the aim parameter in the final possition calculated. Default is
        false to not consider it.
    ...
    150
    >>> calculate_final_position(["forward 5", "down 5", "forward 8", "up 3", "down 8", "forward 2", ], consider_aim=True)
    900
    """
...
if __name__ == "__main__":
    ...
    time2 = time.time()
    final_position2 = calculate_final_position(movements, consider_aim=True)
    time3 = time.time()

    print(f"Part 1 final position: {final_position}. Time: {time1-time0:.8f}")
    print(f"Part 2 final position: {final_position2}. Time: {time3-time2:.8f}")
